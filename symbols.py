...
STRATEGY_UNTESTED = [
    ...
    "WELENT",
    "YATHARTH",
    # ── High-mover additions (ATR%>=2.5, turnover>=10cr): 119 symbols ──
    # Sourced from NSE EQ list beyond NSE Total Market 750. These are
    # smaller stocks selected SPECIFICALLY for daily volatility — top names
    # have ATR 6-12% with reasonable turnover (15-200 cr). Treat as the
    # most-noisy bucket of the universe; alerts here will be more frequent
    # and lower-quality on average than the Nifty 500 names above.
    "AEROFLEX",
    "AGARIND",
    "AGI",
    "AGIIL",
    ...
    "WENDT",
    "YATRA",
    # ── Established-stock additions: 238 NSE EQ names with >=2y history ──
    # User wants >=2y daily history so the 1wk timeframe (which needs ~100
    # weekly bars to identify HTF zones) has enough data. Liquidity gate
    # relaxed from 10cr → 5cr turnover since we're not screening for
    # volatility, just for analyzable established names. No ATR filter.
    "20MICRONS",
    "63MOONS",
    "ABCOTS",
    "ADFFOODS",
